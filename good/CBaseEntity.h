...
public:
	/*vector*/
	NETVAR(Vector, DT_CSPlayer, m_vecOrigin); // lets use this for fl fix hehe
	NETVAR(Vector, DT_CSPlayer, m_vecMaxs);
	NETVAR(Vector, DT_CSPlayer, m_vecMins);
	NETVAR(Vector, DT_CSPlayer, m_viewPunchAngle);
	NETVAR(Vector, DT_CSPlayer, m_aimPunchAngle);
	NETVAR(Vector, DT_CSPlayer, m_angEyeAngles);
	/*float*/
	NETVAR(float, DT_CSPlayer, m_flSimulationTime);
	NETVAR(float, DT_CSPlayer, m_flMaxspeed);
	/*int*/
	NETVAR(int, DT_CSPlayer, m_nTickBase);
	NETVAR(int, DT_CSPlayer, m_fFlags);
	NETVAR(int, DT_CSPlayer, m_hActiveWeapon);
	NETVAR(int, DT_CSPlayer, m_iHealth);
	NETVAR(int, DT_CSPlayer, m_iTeamNum);
	NETVAR(int, DT_CSPlayer, m_nHitboxSet);
	NETVAR(int, DT_CSPlayer, m_hObserverTarget);
	/*byte*/
	NETVAR(byte, DT_CSPlayer, m_lifeState);
	NETVAR(byte, DT_CSPlayer, m_bGunGameImmunity);
	/*vfunc*/
	VFUNCR0(AbsOrigin, 10, Vector&);
	/*functions*/
	...
	Vector& m_vecViewOffset() {
		static int offset = NetVarManager->getOffset("DT_CSPlayer", "m_vecViewOffset[0]");
		return *(Vector*)((DWORD)this + offset);
	}
	Vector& m_vecVelocity() {
		static int offset = NetVarManager->getOffset("DT_CSPlayer", "m_vecVelocity[0]");
		return *(Vector*)((DWORD)this + offset);
	}
	float& m_flOldSimulationTime() {
		static int offset = NetVarManager->getOffset("DT_CSPlayer", "m_flSimulationTime") + 0x4;
		return *(float*)((DWORD)this + offset);
	}
	int GetChockedPackets() {
		if (m_flSimulationTime() > OldSimulationTimes[GetNetworkable()->EntIndex()])
			return TIME_TO_TICKS(fabs(m_flSimulationTime() - OldSimulationTimes[GetNetworkable()->EntIndex()]));
		return 0;
	}
	matrix3x4& m_rgflCoordinateFrame() {
		static int offset = NetVarManager->getOffset("DT_CSPlayer", "m_CollisionGroup") - 0x30;
		return *(matrix3x4*)((DWORD)this + offset);
	}
	Vector& m_vecNetworkOrigin() {
		static int offset = NetVarManager->getOffset("DT_CSPlayer", "m_flFriction") - sizeof(Vector);
		return *(Vector*)((DWORD)this + offset);
	}
	byte& m_MoveType() {
		static int offset = NetVarManager->getOffset("DT_CSPlayer", "m_nRenderType") + 1;
		return *(byte*)((DWORD)this + offset);
	}
};
...
public:
	...
	void	Init(void *pOverlay);

	// float	SetBlending( int iBlender, float flValue, CBaseAnimating *pOwner );
	void	StudioFrameAdvance(float flInterval, void *pOwner);
	void	DispatchAnimEvents(void *eventHandler, void *pOwner);
...
public:
	...
	void RemoveAll(void) { m_animationQueue.clear(); };

public:
	std::vector< CAnimationLayer >	m_animationQueue;
};
...
public:
	...
	void Init(void *pPlayer, const void* config);
	...
	virtual const Vector& GetRenderAngles();
...
public:
	...
	void				GetPoseParameters(void *pStudioHdr, float poseParameter[24]);
...
protected:
	...
	virtual void		ComputePoseParam_BodyPitch(void *pStudioHdr);
